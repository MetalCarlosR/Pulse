...
public class FovEditor : Editor
{
    private void OnSceneGUI()
    {
        ...
        Handles.DrawLine(FOV.transform.position, (new Vector3(FOV.transform.forward.x , FOV.transform.forward.y  , FOV.transform.forward.z + FOV.anguloVision / 2)) * FOV.radioVision);
        Handles.DrawLine(FOV.transform.position, (new Vector3(FOV.transform.forward.x, FOV.transform.forward.y, FOV.transform.forward.z - FOV.anguloVision / 2)) * FOV.radioVision);

        if (FOV.detected) {
            Handles.DrawLine(FOV.transform.position, FOV.targetPos.position);
        }
    }
}
